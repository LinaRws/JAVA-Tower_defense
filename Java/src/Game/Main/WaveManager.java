...
import Game.Entities.Enemies.EnemyType.Aigle;
import Game.Entities.Enemies.EnemyType.Dragon;
...
public void init() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        for (int j = 0; j < numberOfEnemiesInWave; j++) {
            Enemy enemy;
            if (i % 3 == 0) {
                enemy = new Dragon(getRandomRoute());
            } else {
                enemy = new Aigle(getRandomRoute());
            }
            wave.add(enemy);
        }
